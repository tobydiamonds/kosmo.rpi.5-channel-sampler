...
# ---------- SETTINGS -------------
DEFAULT_THRESHOLD = 0.05        # RMS amplitude (0–1) above noise floor
...
class Sampler:
    ...
    def set_on_sound_detected(self, callback):
        self._on_sound_detected = callback

    def _fire_sound_detected(self):
        if self._on_sound_detected is not None:
            self._on_sound_detected(self.current_bank, self.current_channel)

    def set_on_post_processing_started(self, callback):
        self._on_post_processing_started = callback

    def _fire_post_processing_started(self):
        if self._on_post_processing_started is not None:
            self._on_post_processing_started(self.current_bank, self.current_channel)
    ...
    def button_monitor(self):
        ...
        while not self.shutdown.is_set():
           ...
           #print(is_recording, self.rms)
            ...
            if self.rms > self.record_threshold and not is_recording and (now - last_stop_time) > COOLDOWN_SEC:
                # Start recording
            ...
            if is_recording:
                ...
                if self.rms < self.record_threshold:
                    print("Sound dropped below threshold – recording stopped.")
                    self._fire_post_processing_started()
                    self.stop_recording()
    ...
    def trim_silence(self, filename):
        ...
        start_mask = np.abs(mono) > self.record_threshold
        ...
        end_threshold = max(0, self.record_threshold - 0.025)
    ...
    def set_threshold(self, threshold):
        ...
        self.record_threshold = threshold / 10
        print(self.record_threshold)
